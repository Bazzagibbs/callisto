...
parser.add_argument("-name", default=APP_NAME, help="The base name, without file extension, of the application executable")
...
odin_app_name_arg   = f"-define:APP_NAME={args.name}"
...
if build_runner:
    ...
    command = f"odin build {runner_dir} {odin_debug_arg} -out={os.path.join(out_dir, args.name)}{exe_ext} {odin_hot_reload_arg} {odin_app_name_arg}"
    print(command)
...
command = f"odin build . -build-mode=shared {odin_debug_arg} -out={staging_dll_name} {odin_hot_reload_arg}"
print(command)
result = subprocess.run(command, cwd=os.getcwd(), stderr=sys.stderr, stdout=sys.stdout)
if result.returncode == 0:
